...
// // const {data}=await axios.post("https://api.clipdrop.co/text-to-image/v1",formData,{
...
export const generateImage = async (req, res) => {
  ...
  // try to decode buffer responses from axios (arraybuffer)
  const errBuf = error.response?.data;
  const errText = errBuf ? Buffer.from(errBuf).toString("utf8") : error.message;
  console.log("ClipDrop ERROR TEXT →", errText);
...
export const removeImageObject = async (req, res) => {
  try {
    ...
    const imageUrl = cloudinary.url(uploadResult.public_id, {
      transformation: [
        {
          effect: `gen_remove:${object}`, // 👈 remove object
        },
      ],
      resource_type: "image",
    });
    ...
    console.log("OBJECT REMOVE ERROR →", error);
    res.json({
      success: false,
...
const prompt = `Revier the following resume and provide constructive feedback on its strengths, weaknesses, and area for improvement. Resume Content:\n\n${pdfData.text}`;
